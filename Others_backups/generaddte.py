...
if os.path.exists(location):
    ...
    dict1={}
    k=1   
    #bulk_op=conn.db.initialize_ordered_bulk_op()
    document_bulk=""
    for line in logfile:
        ...
        #retval = bulk_op.insert({'id1':id1,'elapsed_time':elapsed_time,'client_ip':client_ip,'username':username,'date':date,'time':time,'httpstatus':httpstatus,'byte_transferred':byte_transferred,'mime':mime,'filter_profile':filter_profile,'interface':interface,'method_url':method_url,'filter_name':filter_name,'user_agent':user_agent1})
...
#print di
    #retval = bulk_op.execute()
    print "Successfully generated reports....Visit <a href='http://localhost:8000/view.html'> View </a> to see reports... exec time"
else:
